...
impl Ptr<Operation> {
    ...
    pub fn walk(
        ...
        let regions: Vec<_> = self.deref(ctx).regions().collect();
        for region in regions {
            let blocks: Vec<Ptr<BasicBlock>> = region.deref(ctx).iter(ctx).collect();
            for block in blocks {
                let block_ops: Vec<Ptr<Operation>> = block.deref(ctx).iter(ctx).collect();
    ...
    where
        T: Op,
    {
        let self_op = Operation::get_op(*self, ctx);
        if order == WalkOrder::PreOrder {
            if let Some(t_op) = self_op.downcast_ref::<T>() {
                match callback(t_op) {
                    WalkResult::Interrupt => return WalkResult::Interrupt,
                    WalkResult::Skip => return WalkResult::Advance,
                    WalkResult::Advance => (),
